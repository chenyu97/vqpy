...
class Car(VObjBase):
    ...
    def color(self, values):
        ...
        if (most_common_element != None) and (count > (REUSE_TIMESCALE_THRESHOLD / 2.0)):
            ...
            image = values["image"]
            reference = ["blue", "green", "black",
                         "white", "red", "grey", "silver", "brown"]
            color = recognize(self.color_detect_transform,
                              self.color_detect_model, image, reference)
            return color
...
class MyQuery(QueryBase):
    def __init__(self, query, recognize_parameters):
        self.car = Car()
        self.query = query
        self.car.set_color_recognize(
            recognize_parameters["color_model"], recognize_parameters["color_transform"])
...
if __name__ == "__main__":
    ...
    print("Querying the video: " + filename)
    ...
    # name_list = ['../../../Three_Datasets/Jackson/cut_videos/3min_jacksonhole_sat_am000.mp4']
    ...
    #     end_time = time.time()
    #     print('query_cost_time: ' + str(end_time - start_time))
    #     with open('./result/Q3_time_cost_' + str(index) + '_' + time.strftime("%Y.%m.%d.%H.%M.%S"), 'a') as file:
